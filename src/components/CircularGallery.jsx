...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // 以中心为基准向两侧对称铺开，避免加宽容器后某一侧留白
    this.x = this.width * (this.index - (this.length - 1) / 2);
  }
}
...
class App {
  ...
  createRenderer() {
    this.renderer = new Renderer({
      alpha: true,
      // 着色器已用 smoothstep 做边缘抗锯齿，关闭 MSAA 省每帧 GPU 开销
      antialias: false,
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      const sync = () => {
        if (intersecting && !document.hidden) app.resume();
        else app.pause();
      };
    ...
    return () => {
      isMounted = false;
      if (app) {
        if (app._cleanupPerf) app._cleanupPerf();
        app.destroy();
      }
    };
